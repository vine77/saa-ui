App.Network = Ember.Object.extend({
  ...
  save: function () {
    ...
    return Ember.$.ajax({
      url: (App.getApiDomain()) + '/api/v3/netconfig',
      type: 'POST',
      data: JSON.stringify(networkData),
      dataType: 'json',
      contentType: 'application/json',
      complete: function (xhr) {
        App.log(xhr.status + ' response from POST /api/v3/netconfig: ' + xhr.statusText);
      },
  ...
  check: function () {
    return Ember.$.ajax({
      ...
      dataType: 'json',
      complete: function (xhr) {
        App.log(xhr.status + ' response from GET /api/v3/netconfig: ' + xhr.statusText);
      },
      success: function (data) {
      ...
      error: function (xhr) {
        ...
        App.log('ERROR ' + xhr.status + ' from GET /api/v3/netconfig: ' + xhr.statusText);
      }
    });
  }
});
