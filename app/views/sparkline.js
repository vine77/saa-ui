...
export default Ember.View.extend({
  ...
  displayGraph: function(selector, width, height, interpolation, animate, updateFrequency) {
    ...
    var graph = window.d3.select(selector).append('svg:svg').attr('width', '100%').attr('height', '100%');
    ...
    var x = window.d3.scale.linear().domain([0, 48]).range([-5, width]);  // Starting point is -5 so first value doesn't show and slides off edge as part of transition
    var y = window.d3.scale.linear().domain([0, 10]).range([0, height]);
    // Set up graph
    var line = window.d3.svg.line().x(function(d, i) {
      return x(i);
    }).y(function(d) {
      return y(d);
