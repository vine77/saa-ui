App.MtWilson = Ember.Object.extend({
  ...
  check: function () {
    return Ember.$.ajax({
      url: (App.getApiDomain()) + '/api/v3/mtwilson/install',
      type: 'GET',
      dataType: "json"
    }).then(function (data, textStatus, xhr) {
      App.log(xhr.status + ' response from GET /api/v3/mtwilson/install: ' + xhr.statusText);
  ...
  install: function () {
    ...
    return Ember.$.ajax({
      ...
      complete: function (xhr, textStatus) {
        App.log(xhr.status + ' response from POST /api/v3/mtwilson/install: ' + xhr.statusText);
        switch (xhr.status) {
          case 201:
  ...
  uninstall: function () {
    return Ember.$.ajax({
      ...
      dataType: "json",
      complete: function (xhr, textStatus) {
        App.log(xhr.status + ' response from DELETE /api/v3/mtwilson/install: ' + xhr.statusText);
        switch (xhr.status) {
          case 200:
            App.mtWilson.set('isInstalled', false);
